...
@section('title', 'Movies - ' . config('app.name'))
...
@section('content')
    ...
    <section class="relative h-[45vh] sm:h-[50vh] min-h-[340px] flex items-end overflow-hidden">
        ...
        <div class="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 pb-10 w-full">
            ...
            <h1 class="text-4xl sm:text-5xl font-extrabold tracking-[-0.02em]">Movies</h1>
            <p class="text-white/40 text-[14px] mt-2 max-w-md">Discover thousands of movies across every genre. Filter, sort, and find your next favorite film.</p>
        </div>
    </section>
    @else
    ...
    <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 {{ $featured ? '-mt-2' : 'mt-4' }}">
        <form method="GET" action="{{ route('movies') }}" id="filter-form" class="mb-8">
            <div class="flex flex-wrap items-center gap-2 sm:gap-3">
                ...
                @if(request()->hasAny(['genre', 'year', 'sort']))
                    <a href="{{ route('movies') }}" class="text-[12px] text-white/30 hover:text-white/60 transition">Clear</a>
                @endif
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover/card:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-3">
                            <div class="flex items-center gap-1.5 mb-1">
                                ...
                                @if($item->release_date)
                                    <span class="text-white/40 text-[10px]">{{ $item->release_date->format('Y') }}</span>
                                @endif
                            </div>
        ...
        <div class="text-center py-20">
            ...
            <p class="text-white/30 text-[15px]">No movies found matching your filters.</p>
            <a href="{{ route('movies') }}" class="text-red-400/70 text-[13px] hover:text-red-400 transition mt-2 inline-block">Clear all filters</a>
        </div>
        @endif
    </div>
@endsection
